...
class RegistroApp(tk.Tk):
    def __init__(self):
        ...
        # Buscador por nombre
        buscar_label = tk.Label(self, text="Buscar por nombre:")
        buscar_label.pack()
        self.buscar_entry = tk.Entry(self)
        self.buscar_entry.pack()
        
        buscar_button = tk.Button(self, text="Buscar", command=self.buscar_nombre)
    ...
    def buscar_nombre(self):
        nombre_buscado = self.buscar_entry.get()
        self.tabla.selection_remove(self.tabla.selection())
        if nombre_buscado:
            for item in self.tabla.get_children():
                valores = self.tabla.item(item)["values"]
                if valores[0].lower() == nombre_buscado.lower():
                    self.tabla.selection_add(item)
                    self.tabla.focus(item)
                    self.tabla.see(item)
                    break
            else:
                messagebox.showinfo("Búsqueda", f"No se encontraron resultados para '{nombre_buscado}'")
        else:
            messagebox.showerror("Búsqueda", "Por favor, ingrese un nombre para buscar.")
